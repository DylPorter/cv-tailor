...
### Task 12: Upload parsing util (PDF + .docx)

Dad's real CVs are text PDFs (verified). Parse both PDF (via `pdfjs-dist`) and `.docx` (via `mammoth`); reject other types with a friendly message. Paste remains the universal fallback.
...
Expected: FAIL — `parseUploadFile` not found.
...
// Vite resolves ?url to the worker file path; pdfjs needs a worker in the browser.
...
Note on the worker import: `pdfjs-dist/build/pdf.worker.min.mjs?url` is a Vite-only `?url` import. If vitest fails to resolve it during the test run, set `workerSrc` lazily inside `parsePdf` instead (the test mocks `pdfjs-dist`, so the worker is never actually used in tests) — keep the public `parseUploadFile` signature and the three test cases. Report any such adaptation.
...
```bash
git add src/lib/parseUpload.ts src/lib/parseUpload.test.ts package.json package-lock.json
git commit -m "feat: PDF + .docx upload parsing"
```

---

### Task 12b: Data backup (export/import) in storage layer

Persistence is browser `localStorage`; this adds a JSON backup/restore so a cache-clear or device-switch can't lose the master profile + saved folder. No server-side storage.

**Files:**
- Modify: `src/store/storage.ts` (add `exportData`, `importData`)
- Test: `src/store/storage.test.ts` (add cases)

- [ ] **Step 1: Add the failing tests** to `src/store/storage.test.ts`
...
```
Add `exportData, importData` to the existing import line from `./storage`.

- [ ] **Step 2: Run to verify it fails**

Run: `npm run test -- src/store/storage.test.ts`
Expected: FAIL — `exportData`/`importData` not found.

- [ ] **Step 3: Implement** (append to `src/store/storage.ts`)
...
```bash
git add src/store/storage.ts src/store/storage.test.ts
git commit -m "feat: localStorage export/import backup"
```
